...
Status: Discovery + architecture scaffolded
...
- `YouTubeCompliantDiscoveryAdapter`
  - Discovery placeholder only.
  - Returns explicit `discovery_not_configured` until official API auth is wired.
  - Playback intentionally returns `null` and `false`.
